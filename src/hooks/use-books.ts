...
import { allBooks, type Book } from '@/lib/data';
...
export function useBooks() {
  ...
  const fetchBooksFromStorage = useCallback(() => {
    try {
      ...
      if (storedBooks) {
        ...
        setBooks(allBooks);
        localStorage.setItem(BOOKS_STORAGE_KEY, JSON.stringify(allBooks));
      }
    } catch (error) {
  ...
  const addBook = (book: Omit<Book, 'id'>): { success: boolean; message?: string } => {
    const currentBooks = JSON.parse(localStorage.getItem(BOOKS_STORAGE_KEY) || '[]');
    
    const newBook: Book = {
        ...book,
        id: Date.now(), // Use timestamp for a unique enough ID
    }
